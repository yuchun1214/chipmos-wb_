...
#define device_malloc_evolution_factor(factor, type, amount_of_chromosomes)\
	cudaCheck(cudaMalloc((void**)&(factor), sizeof(type) * (amount_of_chromosomes)), "cudaMalloc for " #factor);

#define host_malloc_evolution_factor(factor, type, amount_of_chromosomes)\
	cudaCheck(cudaMallocHost((void**)&(factor), sizeof(type) * (amount_of_chromosomes)), "cudaMalloc for " #factor);

#define cpy_factor_h2d(dest, src, type, amount_of_chromosomes)\
    cudaCheck(cudaMemcpy(dest, src, sizeof(type)*(amount_of_chromosomes), cudaMemcpyHostToDevice), "cudaMemcpy for " #src " from host to device");


__global__ void initOps(list_operations_t * list_ops, job_base_operations_t * job_ops, machine_base_operations_t * machine_ops){
...
__global__ void initializeJobs(job_t **jobs, process_time_t **process_times, job_base_operations_t *job_ops, chromosome_base_t *chromosomes, int AMOUNT_OF_JOBS, int AMOUNT_OF_R_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(x < AMOUNT_OF_R_CHROMOSOMES && y < AMOUNT_OF_JOBS){
        job_initialize(&jobs[x][y]);
        job_ops->set_process_time(&jobs[x][y].base, process_times[y], jobs[x][y].base.size_of_process_time);
        job_ops->set_ms_gene_addr(&jobs[x][y].base, chromosomes[x].ms_genes + y);
        job_ops->set_os_gene_addr(&jobs[x][y].base, chromosomes[x].os_genes + y);
    }
}
...
__global__ void initializeChromosomes(chromosome_base_t *chromosomes, const int AMOUNT_OF_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x < AMOUNT_OF_CHROMOSOMES){
        chromosomes[x].chromosome_no = x;
        chromosomes[x].fitnessValue = 0;
...
__global__ void machineSelection(job_t **jobs, job_base_operations_t *jbops, int AMOUNT_OF_JOBS, int AMOUNT_OF_R_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if( x < AMOUNT_OF_R_CHROMOSOMES && y < AMOUNT_OF_JOBS){
        ...
        jobs[x][y].base.ptime = jobs[x][y].base.process_time[machine_idx].process_time;
    }
}

__global__ void machineSelection2(job_t **jobs, machine_t **machines, machine_base_operations_t *ops, int AMOUNT_OF_JOBS, int AMOUNT_OF_MACHINES, int AMOUNT_OF_R_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void getMachineInformation(machine_t *machines, unsigned int *machine_numbers, unsigned int *sizeof_jobs, int AMOUNT_OF_MACHINES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void getMachineJobs(machine_t *machines, double ** data, int AMOUNT_OF_MACHINES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    if(x < AMOUNT_OF_MACHINES){
...
__global__ void sortJob(machine_t **machines, list_operations_t * list_ops, machine_base_operations_t *mbops, int AMOUNT_OF_MACHINES, int AMOUNT_OF_R_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if( x < AMOUNT_OF_R_CHROMOSOMES && y < AMOUNT_OF_MACHINES){
...
__global__ void scheduling(machine_t **machines, job_base_operations_t *job_ops, machine_base_operations_t *machine_ops, int AMOUNT_OF_MACHINES, int AMOUNT_OF_R_CHROMOSOMES){
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    if(x < AMOUNT_OF_R_CHROMOSOMES && y < AMOUNT_OF_MACHINES){
...
__global__ void sortChromosomes(chromosome_base_t *chromosomes, int AMOUNT_OF_CHROMOSOMES){
...
void cpyEvolutionFactors(struct evolution_factors_t *dest, struct evolution_factors_t *src, int AMOUNT_OF_CHROMOSOMES){
    cpy_factor_h2d(dest->c_selected1, src->c_selected1, unsigned int, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->c_selected2, src->c_selected2, unsigned int, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->m_selected, src->m_selected, unsigned int, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->cut_points, src->cut_points, unsigned int, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->range, src->range, unsigned int, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->new_genes, src->new_genes, double, AMOUNT_OF_CHROMOSOMES); 
    cpy_factor_h2d(dest->gene_idx, src->gene_idx, unsigned int, AMOUNT_OF_CHROMOSOMES); 
}
...
void cpyResult(population_t * pop, char *filename){
    ...
    job_t *jobs;
    cudaCheck(cudaMallocHost((void**)&jobs, sizeof(job_t) * AMOUNT_OF_JOBS), "cudaMallocHost for jobs");
    cudaCheck(cudaMemcpy(jobs, pop->host_objects.address_of_cujobs[0], sizeof(job_t)*AMOUNT_OF_JOBS, cudaMemcpyDeviceToHost), "cudaMemcpy jobs from d 2 h");

    FILE * file = fopen(filename, "w");
    for(int i = 0; i < AMOUNT_OF_JOBS; ++i){
        fprintf(file, "%s, %s, %s, %s, %.3f, %.3f\n", jobs[i].base.job_info.data.text, jobs[i].part_no.data.text, jobs[i].part_id.data.text, convertUIntToEntityName(jobs[i].base.machine_no).c_str(), jobs[i].base.start_time, jobs[i].base.end_time);
    }
...
void initializePopulation(population_t * pop){
    ...
    cudaCheck(cudaMallocHost((void**)&address_of_process_times, sizeof(process_time_t*)*AMOUNT_OF_JOBS), "cudaMallocHost for address of process times");
    for(int i = 0; i < AMOUNT_OF_JOBS; ++i){
        process_time_t * entry;
        cudaCheck(cudaMalloc((void**)&entry, sizeof(process_time_t)*pop->task.size_of_process_times[i]), "cudaMalloc for an entry of process times");
        cudaCheck(cudaMemcpy(entry, pop->task.process_times[i], sizeof(process_time_t)*pop->task.size_of_process_times[i], cudaMemcpyHostToDevice), "cudaMemcpy for an entry of process time");
        address_of_process_times[i] = entry;
    }
    cudaCheck(cudaMalloc((void**)&process_times, sizeof(process_time_t*)*AMOUNT_OF_JOBS), "cudaMalloc for process times");
    cudaCheck(cudaMemcpy(process_times, address_of_process_times, sizeof(process_time_t*)*AMOUNT_OF_JOBS, cudaMemcpyHostToDevice), "cudaMemcpy for process times");
    ...
    double * tmp_genes;
    cudaCheck(cudaMallocHost((void**)&tmp_genes, sizeof(double)*(AMOUNT_OF_JOBS<<1)), "cudaMallocHost for genes sample");
    cudaCheck(cudaMalloc((void**)&chromosomes, sizeof(chromosome_base_t)*AMOUNT_OF_R_CHROMOSOMES), "cudaMalloc for chromosomes");
    cudaCheck(cudaMallocHost((void**)&address_of_cu_genes, sizeof(double *)*AMOUNT_OF_R_CHROMOSOMES), "cudaMallocHost for address of genes");
    for(int i = 0; i < AMOUNT_OF_R_CHROMOSOMES; ++i){
        double * tmp;
        random(tmp_genes, AMOUNT_OF_JOBS << 1);
        cudaCheck(cudaMalloc((void**)&tmp, sizeof(double)*(AMOUNT_OF_JOBS << 1)), "cudaMalloc for entry of genes");
        cudaCheck(cudaMemcpy(tmp, tmp_genes, sizeof(double)*(AMOUNT_OF_JOBS << 1), cudaMemcpyHostToDevice), "cudaMemcpy for genes");
        address_of_cu_genes[i] = tmp;
    }
    cudaCheck(cudaMalloc((void**)&genes, sizeof(double*)*AMOUNT_OF_R_CHROMOSOMES), "cudaMalloc for genes");
    cudaCheck(cudaMemcpy(genes, address_of_cu_genes, sizeof(double*)*AMOUNT_OF_R_CHROMOSOMES, cudaMemcpyHostToDevice), "cudaMemcpy genes from host to device");

    // setup host chromosomes
    chromosome_base_t * host_chromosomes;
    cudaCheck(cudaMallocHost((void**)&host_chromosomes, sizeof(chromosome_base_t)*pop->parameters.SWAP_CHROMOSOMES), "cudaMallocHost for host chromosomes");
    ...
    // device
	device_malloc_evolution_factor(pop->evolution_factors.device.c_selected1, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	device_malloc_evolution_factor(pop->evolution_factors.device.c_selected2, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
    device_malloc_evolution_factor(pop->evolution_factors.device.cut_points, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
    device_malloc_evolution_factor(pop->evolution_factors.device.range, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	device_malloc_evolution_factor(pop->evolution_factors.device.m_selected, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	device_malloc_evolution_factor(pop->evolution_factors.device.new_genes, double, AMOUNT_OF_R_CHROMOSOMES);
	device_malloc_evolution_factor(pop->evolution_factors.device.gene_idx, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	// host
	host_malloc_evolution_factor(pop->evolution_factors.host.c_selected1, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	host_malloc_evolution_factor(pop->evolution_factors.host.c_selected2, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
    host_malloc_evolution_factor(pop->evolution_factors.host.cut_points, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	host_malloc_evolution_factor(pop->evolution_factors.host.range, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	host_malloc_evolution_factor(pop->evolution_factors.host.m_selected, unsigned int, AMOUNT_OF_R_CHROMOSOMES);
	host_malloc_evolution_factor(pop->evolution_factors.host.new_genes, double, AMOUNT_OF_R_CHROMOSOMES);
	host_malloc_evolution_factor(pop->evolution_factors.host.gene_idx, unsigned int, AMOUNT_OF_R_CHROMOSOMES);



    // setup ops
    list_operations_t * list_ops;
    cudaCheck(cudaMalloc((void**)&list_ops, sizeof(list_operations_t)), "cudaMalloc for list operations");
    job_base_operations_t * job_ops;
    cudaCheck(cudaMalloc((void**)&job_ops, sizeof(job_base_operations_t)), "cudaMalloc for job operations");
    machine_base_operations_t * machine_ops;
    cudaCheck(cudaMalloc((void**)&machine_ops, sizeof(machine_base_operations_t) + sizeof(setup_time_func_t) * 7), "cudaMalloc for machine operations");
    ...
    dim3 machine_chromosome_block(AMOUNT_OF_R_CHROMOSOMES / machine_chromosome_thread.x, AMOUNT_OF_MACHINES / machine_chromosome_thread.y + 1);
    ...
    initializeChromosomes<<<AMOUNT_OF_R_CHROMOSOMES, 1>>>(chromosomes, genes, AMOUNT_OF_JOBS, AMOUNT_OF_R_CHROMOSOMES);  
    initializeJobs<<<job_chromosome_block, job_chromosome_thread>>>(jobs, process_times, job_ops, chromosomes, AMOUNT_OF_JOBS, AMOUNT_OF_R_CHROMOSOMES);
    initializeMachines<<<machine_chromosome_block, machine_chromosome_thread>>>(machines, tools, wires, AMOUNT_OF_MACHINES, AMOUNT_OF_R_CHROMOSOMES); 
...
void testMachineInformation(population_t * pop, int n){
    ...
    cudaCheck(cudaMalloc((void**)&d_machine_nums, array_size), "cudaMalloc for d_machine_nums");
    cudaCheck(cudaMallocHost((void**)&h_machine_nums, array_size), "cudaMalloc for h_machine_nums"); 
    cudaCheck(cudaMalloc((void**)&d_sizeof_jobs, array_size), "cudaMalloc for d_sizeof_jobs");
    cudaCheck(cudaMallocHost((void**)&h_sizeof_jobs, array_size), "cudaMalloc for h_sizeof_jobs"); 
    
    getMachineInformation<<<AMOUNT_OF_MACHINES, 1>>>(pop->host_objects.address_of_cumachines[n], d_machine_nums, d_sizeof_jobs, AMOUNT_OF_MACHINES);

    cudaCheck(cudaMemcpy(h_machine_nums, d_machine_nums, array_size, cudaMemcpyDeviceToHost), "cudaMemcpy from device to host for machine_num");
    cudaCheck(cudaMemcpy(h_sizeof_jobs, d_sizeof_jobs, array_size, cudaMemcpyDeviceToHost), "cudaMemcpy from device to host for sizeof_jobs");

    double ** genes;
    double ** address_of_genes;
    cudaCheck(cudaMallocHost((void**)&address_of_genes, sizeof(double*)*AMOUNT_OF_MACHINES), "cudaMallocHost for entry of genes in testMachineInformation");
    cudaCheck(cudaMalloc((void**)&genes, sizeof(double*)*AMOUNT_OF_MACHINES), "cudaMallocHost for entries of genes in testMachineInformation");
    for(int i = 0; i < AMOUNT_OF_MACHINES; ++i){
        double * entry;
        cudaCheck(cudaMalloc((void**)&entry, sizeof(double)*h_sizeof_jobs[i]), "cudaMalloc for entry of genes");
        address_of_genes[i] = entry;
    }
    cudaCheck(cudaMemcpy(genes, address_of_genes, sizeof(double*)*AMOUNT_OF_MACHINES, cudaMemcpyHostToDevice), "cudaMemcpy for genes from host to device");

    // for(int i = 0; i < AMOUNT_OF_MACHINES; ++i){
    //     printf("%s : %u\n", convertUIntToEntityName(h_machine_nums[i]).c_str(), h_sizeof_jobs[i]);
    // }
    getMachineJobs<<<AMOUNT_OF_MACHINES, 1>>>(pop->host_objects.address_of_cumachines[n], genes, AMOUNT_OF_MACHINES); 
    for(int i = 0; i < AMOUNT_OF_MACHINES; ++i){
        printf("%s[%d] : ", convertUIntToEntityName(h_machine_nums[i]).c_str(), h_sizeof_jobs[i]);
        double * entry;
        if(h_sizeof_jobs[i] == 0){
            printf("\n");
            continue;
        }
        cudaCheck(cudaMallocHost((void**)&entry, sizeof(double)*h_sizeof_jobs[i]), "cudaMallocHost for entry of genes");
        cudaCheck(cudaMemcpy(entry, address_of_genes[i], sizeof(double)*h_sizeof_jobs[i], cudaMemcpyDeviceToHost), "cudaMemcpy entry from device to host");
...
void *geneticAlgorithm(void *_pop){
    ...
    cudaCheck(cudaMallocHost((void**)&chrs, sizeof(chromosome_base_t)*AMOUNT_OF_R_CHROMOSOMES), "cudaMalloc for chromosomes for testing");
    ...
    initializeChromosomes<<<AMOUNT_OF_R_CHROMOSOMES, 1>>>(pop->chromosomes.chromosomes, AMOUNT_OF_R_CHROMOSOMES);  
    ...
    for(int i = 0 ; q1 < q2; ++i, q1 = clock()){
        ...
        generateCrossoverFactors(&pop->evolution_factors.host, CROSSOVER_AMOUNT>>1, AMOUNT_OF_JOBS << 1, AMOUNT_OF_CHROMOSOMES);
        generateMutationFactors(&pop->evolution_factors.host, MUTATION_AMOUNT, AMOUNT_OF_JOBS << 1, AMOUNT_OF_CHROMOSOMES);
        cpyEvolutionFactors(&pop->evolution_factors.device, &pop->evolution_factors.host, AMOUNT_OF_CHROMOSOMES);
        ...
        // cudaDeviceSynchronize();
   	    ...
   	    crossover<<<1, CROSSOVER_AMOUNT>>>(
                ...
                AMOUNT_OF_JOBS, CROSSOVER_AMOUNT >> 1,
        ...
        mutation<<<1, MUTATION_AMOUNT>>>(
                ...
                AMOUNT_OF_JOBS, MUTATION_AMOUNT,
                AMOUNT_OF_CHROMOSOMES); 
        cudaStreamSynchronize(0);
    }
...
void swapPopulation(population_t pops[], const int AMOUNT_OF_POPULATIONS){
    for(int i = 0; i < AMOUNT_OF_POPULATIONS - 1; ++i){
        cudaCheck(cudaMemcpy(pops[i + 1].chromosomes.host_chromosomes, pops[i].chromosomes.chromosomes, sizeof(chromosome_base_t)*pops[i].parameters.SWAP_CHROMOSOMES, cudaMemcpyDeviceToHost), "cudaMemcpy d2h for chromosomes");
    }

    cudaCheck(cudaMemcpy(pops[0].chromosomes.host_chromosomes, pops[AMOUNT_OF_POPULATIONS - 1].chromosomes.chromosomes, sizeof(chromosome_base_t)*pops[0].parameters.SWAP_CHROMOSOMES, cudaMemcpyDeviceToHost), "cudaMemcpy d2h for first population");

    // copy to device
    for(int i = 0; i < AMOUNT_OF_POPULATIONS; ++i){
        cudaCheck(cudaMemcpy(pops[i].chromosomes.chromosomes, pops[i].chromosomes.host_chromosomes, sizeof(chromosome_base_t)*pops[i].parameters.SWAP_CHROMOSOMES, cudaMemcpyHostToDevice), "cudaMemcpy h2d for chromosomes");
    }
